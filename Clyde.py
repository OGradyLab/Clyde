...
def create_motor_control(page, motor_index, row):
    ...
    direction_button.grid(row=0, column=2, padx=5, pady=5)
...
welcome_label.grid(row=1, column=1, columnspan=3, pady=0)

for i in range(6):
    create_motor_control(frame, i, i+1)

# Create two additional buttons
stop_all_motors_button = tk.Button(frame, text="Stop All\nMotors", command=stop_all_motors, bg='blue', fg='white', font=("Arial", 18, "bold"))
stop_all_motors_button.grid(row=2, column=1,columnspan=3, padx=10, pady=2)

quit_program_button = tk.Button(frame, text="Quit\nProgram", command=quit_program, bg='orangered', fg='white', font=("Arial", 18, "bold"))
quit_program_button.grid(row=3, column=1, padx=15, pady=5)
